...
void high_int (void)
{
  ...
  if(INTCONbits.TMR0IF)
  {
    ...
    {
      //so faz loop se já não está sendo tratado
      if(!gucQtdItensCallbackTickTimerSendoChamados)
      {
        ...
        //indica que não esta mais tratando
  ...
  if(INTCONbits.INT0IF || ucFlagTimeout || !gucPassoNoEstado )
  {
    ...
    switch(gucMaqGeral)
    {
      ...
      {   
        ...
        //muda maquina geral para parada e chama funcao de callback que deverá
      ...
      {
#ifdef SERIALSINC_APENASENTRADA
        //como é bidirecional, pura direto apra evento de byte enviado
        //muda maq para esperda do ack e poe um timeout minimo para em seguida
        //sair deste estado (como se o ACK não fosse recebido)
        ...
        {  
          case MAQENV_INICIANDO: 
            ...
            //muda portas para modo de saida (pois é bidirecional)
            SERIAL_CLK_TRIS = 0;
            SERIAL_DATA_TRIS = 0;
          ...
          case MAQENV_FIMIDLE:
            ...
            //fim da sinalziação que libera o device para enviar            
...
#ifdef SERIALSINC_APENASENTRADA
  //poe pinos como entradas pois apenas isto é permitido
  SERIAL_CLK_TRIS = 1;
  SERIAL_DATA_TRIS = 1;
#else
